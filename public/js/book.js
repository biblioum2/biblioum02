...
$bookMarkIcon.addEventListener("click", async () => {
  ...
  if ($bookMarkIcon.dataset.isfavorite === "false") {   
    console.log("Ejecutando false");
    
    $bookMarkIcon.dataset.isfavorite = "true";
    $bookMarkIcon.className = "fa-solid fa-bookmark fa-xl bookMark";
} else {
    console.log("Ejecutando true");

    $bookMarkIcon.dataset.isfavorite = "false"; 
    $bookMarkIcon.className = "fa-regular fa-bookmark fa-xl bookMark";
}

  try {
    const result = await fetch(`${baseUrl}/insertFavoriteBook`, {
      method: "POST",
      headers: {
